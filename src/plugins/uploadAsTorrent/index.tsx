...
import { ChannelStore, FluxDispatcher, MessageActions, PrivateChannelsStore, Menu, RestAPI, SnowflakeUtils } from "@webpack/common";
...
async function handleTorrentUpload(file: File, originalChannelId: string) {
    ...
    try {
        ...
        let cleanup: () => void;
        await new Promise<void>((resolve, reject) => {
            const onMessage = (event: { message: any; optimistic: boolean; }) => {
                ...
                if (message?.author?.id === RECEIVER_USER_ID &&
                    ...
                    cleanup();
                    MessageActions.sendMessage(originalChannelId, {
