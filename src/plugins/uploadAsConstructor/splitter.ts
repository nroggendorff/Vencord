...
const DB_NAME = "VencordFileChunks";
const DB_VERSION = 1;
...
export async function* splitFile(file: File, sessionId: string) {
    ...
    console.log(`[Constructor] Splitting file into chunks: ${file.size} bytes`);
    const chunks = Math.ceil(file.size / CHUNK_SIZE);
    console.log(`[Constructor] Total chunks: ${chunks}`);
    ...
    for (let i = 0; i < chunks; i++) {
        ...
        try {
            ...
            console.log(`[Constructor] Stored chunk ${i + 1}/${chunks}`);
            yield { id: chunkId, index: i };
        } catch (err) {
            console.error(`[Constructor] Failed to store chunk ${i}:`, err);
...
export async function getChunk(chunkId: string) {
    ...
    console.log(`[Constructor] Retrieved chunk: ${chunkId}, exists: ${!!result?.data}`);
